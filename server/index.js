...
async function demarrerServeur () {
				...
				if (err || !donnees) {
					...
					Promise.all(donneesPadsDossiers).then(function (padsSupprimes) {
						...
						db.hset('utilisateurs:' + identifiant, 'dossiers', JSON.stringify(dossiers), function () {
							res.json({ padsCrees: padsCrees, padsRejoints: padsRejoints, padsAdmins: padsAdmins, padsFavoris: padsFavoris, dossiers: dossiers, affichage: donnees.affichage, classement: donnees.classement })
						})
					})
				}
			})
				...
				if (resultat === 1) {
					...
					Promise.all([donneesPad, blocsPad, activitePad]).then(async function (donnees) {
						if (donnees.length > 0 && donnees[0].hasOwnProperty('id')) {
							...
							if (!parametres.pad.fond.includes('/img/') && parametres.pad.fond.substring(0, 1) !== '#' && await fs.pathExists(path.join(__dirname, '..', '/static' + parametres.pad.fond))) {
								await fs.copy(path.join(__dirname, '..', '/static' + parametres.pad.fond), path.normalize(chemin + '/' + id + '/fichiers/' + parametres.pad.fond.split('/').pop(), { overwrite: true }))
					...
					if (typeof donnees === 'object' && donnees !== null && donnees.hasOwnProperty('pad') && donnees.hasOwnProperty('blocs') && donnees.hasOwnProperty('activite')) {
						...
						await fs.writeFile(path.normalize(chemin + '/' + id + '/index.html'), html, 'utf8')
						if (!donnees.pad.fond.includes('/img/') && donnees.pad.fond.substring(0, 1) !== '#' && await fs.pathExists(path.join(__dirname, '..', '/static' + donnees.pad.fond))) {
							await fs.copy(path.join(__dirname, '..', '/static' + donnees.pad.fond), path.normalize(chemin + '/' + id + '/fichiers/' + donnees.pad.fond.split('/').pop(), { overwrite: true }))
					...
					if (donnees.hasOwnProperty('pad') && donnees.hasOwnProperty('blocs') && donnees.hasOwnProperty('activite') && donnees.pad.hasOwnProperty('id') && donnees.pad.hasOwnProperty('token') && donnees.pad.hasOwnProperty('titre') && donnees.pad.hasOwnProperty('identifiant') && donnees.pad.hasOwnProperty('fond') && donnees.pad.hasOwnProperty('acces') && donnees.pad.hasOwnProperty('contributions') && donnees.pad.hasOwnProperty('affichage') && donnees.pad.hasOwnProperty('fichiers') && donnees.pad.hasOwnProperty('liens') && donnees.pad.hasOwnProperty('documents') && donnees.pad.hasOwnProperty('commentaires') && donnees.pad.hasOwnProperty('evaluations') && donnees.pad.hasOwnProperty('date') && donnees.pad.hasOwnProperty('colonnes') && donnees.pad.hasOwnProperty('bloc') && donnees.pad.hasOwnProperty('activite')) {
						db.get('pad', async function (err, resultat) {
							...
							Promise.all(donneesBlocs).then(async function (blocs) {
								...
								if (!donnees.pad.fond.includes('/img/') && donnees.pad.fond.substring(0, 1) !== '#' && await fs.pathExists(path.normalize(cible + '/fichiers/' + donnees.pad.fond.split('/').pop()))) {
									await fs.copy(path.normalize(cible + '/fichiers/' + donnees.pad.fond.split('/').pop()), path.normalize(chemin + '/' + donnees.pad.fond.split('/').pop(), { overwrite: true }))
								}
								const multi = db.multi()
								multi.incr('pad')
	...
	io.on('connection', function (socket) {
		...
		socket.on('connexioncompte', function (session) {
			socket.request.session.identifiant = session.identifiant
			socket.request.session.nom = session.nom
			socket.request.session.email = session.email
			socket.request.session.langue = session.langue
			socket.request.session.statut = session.statut
